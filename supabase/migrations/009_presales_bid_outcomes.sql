...
check (outcome in ('Pending','Passed','Failed','Not Applicable'));

drop function if exists public.update_presales_stage(bigint,text,text,date,date,date,date,boolean,text);
...
begin
  ...
  update public.presales_stage_history set status=p_status,outcome=v_outcome,assigned_to=coalesce(p_assigned_to,''),
    planned_start=p_planned_start,due_date=p_due_date,actual_start=p_actual_start,completed_date=p_completed_date,
    documents_complete=coalesce(p_documents_complete,false),notes=coalesce(p_notes,''),updated_by=auth.uid(),
  ...
  if v_stage.sequence_no in (7,10,11) and p_status='Completed' and v_outcome='Failed' then
    ...
    update public.presales_stage_history h set status='Skipped',outcome='Not Applicable',notes=case when h.notes='' then 'Automatically skipped after unsuccessful bid outcome.' else h.notes end,
      updated_by=auth.uid(),updated_by_email=coalesce(auth.jwt()->>'email',''),updated_at=now()
    from public.presales_stage_templates t where h.project_id=v_history.project_id and h.stage_id=t.id and t.sequence_no>v_stage.sequence_no and h.status not in ('Completed','Skipped');
